...
export const categoryRoute = new Hono()
  .post("/", authGuard(), jsonValidator(addCategoryDto), async (c) => {
  ...
  .get("/", authGuard(), queryValidator(getCategoriesDto), async (c) => {
  ...
  .patch("/:id", authGuard(), jsonValidator(updateCategoryDto), async (c) => {
